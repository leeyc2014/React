...
export default function MyFestGallery() {
    ...
    const getFetchData = async () => {
        ...
        let url = `https://apis.data.go.kr/6260000/FestivalService/getFestivalKr?serviceKey=${apikey}&pageNo=1&numOfRows=20&resultType=json`;
    ...
    // const handleClick = (gugun) => {
    //     const filtered = fdata.filter(item => gugun == item.GUGUN_NM);
        ...
        <div className="w-full h-full flex flex-col justify-start items-center p-5">
            <div className="w-9/10 shadow-lg">
                ...
                <div className=" flex flex-row justify-center items-center-center gap-5 p-5">
                    {/* <select onChange={(e) => handleClick(e.target.value)} className="border border-gray-300 w-2/5"> */}
                    <select ref={selectedGugun} className="border border-gray-300 w-2/5">                 
